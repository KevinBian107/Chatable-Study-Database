...
def transform_text(text):
    ...
    grouped_text = text.groupby("Time", as_index=False).agg(
        {
            ...
            "Study Materials (Lecture Counted)": lambda x: " | ".join(
                x.dropna().astype(str)
            ),
        }
    )
    ...
    grouped_text["Study Materials Combined"] = grouped_text.apply(
        lambda row: " | ".join(
            filter(
                None, [row["Study Materials"], row["Study Materials (Lecture Counted)"]]
            )
        ),
        axis=1,
    )
    ...
    grouped_text = grouped_text.drop(
        columns=["Study Materials", "Study Materials (Lecture Counted)"]
    )

    # Rename 'Study Materials Combined' back to 'Study Materials' if desired
    grouped_text = grouped_text.rename(
        columns={"Study Materials Combined": "Study Materials"}
    )

    grouped_text["Study Materials"] = (
        grouped_text["Time"].astype(str)
        + " | "
        + grouped_text["Quarter"].astype(str)
        + " | "
        + grouped_text["Month"].astype(str)
        + " | "
        + grouped_text["Study Materials"]
    )
