...
def initialize_faiss_vector_store(embedding_df):
    ...
    metadatas = embedding_df.drop(["embedding", "chunk"], axis=1).to_dict(
        orient="records"
    )
    ...
    logging.info(
        f"FAISS vector store initialized with {vector_store.index.ntotal} vectors."
    )
    return vector_store
...
def main():
    ...
    qa_chain = create_retrieval_chain(
        llm=chat, vector_store=vector_store, search_kwargs={"k": 5}
    )
    ...
    while True:
        ...
        quarter = input(
            "Enter the quarter (e.g., '2022Q4') or press Enter to skip: "
        ).strip()

        if quarter:
            logging.info(f"Filtering data for quarter: {quarter}")
            # Define search_kwargs with filter
            search_kwargs = {"k": 5, "filter": {"quarter": quarter}}
            # Create a new QA chain with the filtered retriever
            filtered_qa_chain = create_retrieval_chain(
                llm=chat, vector_store=vector_store, search_kwargs=search_kwargs
            )
